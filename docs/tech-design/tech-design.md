...
| 过载保护 | **有界并发+排队→429** vs 无界 vs 全局令牌桶 | 无界→资源耗尽；进程内信号量简单够用(单实例)；全局令牌桶需共享存储(多实例才需) | 进程内有界并发+有界等待，超限 429(单实例；多实例后续) |
| 缓存/去重 | **磁盘 LRU 缓存 + single-flight** vs 无缓存 vs 内存缓存 | 无缓存→重复下载浪费+惊群；内存缓存受限内存且重启即失；磁盘 LRU 跨重启存活、成本低 | 磁盘 LRU + single-flight（可 -cache-size 0 关闭） |
...
多人并发下，重操作 `/download`（下载 + ffmpeg 合并，占 CPU/磁盘/带宽）若无界并发会耗尽资源。策略：

- **有界并发**：进程内信号量限制同时进行的 `/download` 数（`-max-concurrent`，默认 4）；`/api/info`（轻）不受此闸门，保证信息查询在下载饱和时仍可用。
- **有界排队**：无空位时最多等待 `-max-wait`（默认 5s，ctx 感知）；腾出即继续，超时即拒绝。
- **拒绝语义**：饱和/等待超时 → `429 Too Many Requests` + `Retry-After:<秒>` + `{"code":-429,"message":"server busy"}`。
- **取消传播**：客户端断开 → `r.Context()` 取消 → 正在进行的 fetch 与 `ffmpeg`（`exec.CommandContext`）中止、槽位释放，不空占资源。
- **单次超时**：`-download-timeout`（默认 30m，0=不限）防卡死。
- **优雅关闭**：SIGINT/SIGTERM → 停止接收、在 `-shutdown-grace`（默认 30s）内排空在途，超时才强退；配 `http.Server` 的 `ReadHeaderTimeout`/`IdleTimeout`（不设 `WriteTimeout` 以免切断长下载，改用 per-request ctx 超时）。
- **边界**：限流器属**交付层**关注点，领域/应用层不感知。本期为**单实例**进程内治理；多实例需网关/共享限流（见 §11 OQ2）。

### 7.9 缓存与去重（WS-C）

同一视频重复请求会重复下载+合并，浪费 CPU/磁盘/带宽并拉高时延；并发相同请求还会各做一遍（惊群）。策略（仅 HTTP `serve`；CLI `dl` 直接写 `-o`，不缓存）：

- **内容缓存**：按 `(bvid, page, qn, codec)` 键缓存合并后的 mp4 到磁盘（`-cache-dir`，默认系统临时目录下 `bili2go-cache`）；命中直接回传，**不触发上游**、**不占用限流槽**。
- **去重（single-flight）**：并发相同键只跑一次 produce，其余等待共享结果；与 §7.8 协作——N 个并发相同请求只消耗 1 个下载槽。
- **淘汰**：按总字节 LRU（`-cache-size`，默认 2GiB）；超限逐出最久未用条目（连同 sidecar 元数据）。
- **元数据 sidecar**：每条目存 `.json`（标题/清晰度/编码），命中即可设响应头，无需重解析；启动扫描目录重建索引（**缓存跨重启存活**）。
- **限流协作**：`Acquire` 移入 produce（仅 miss 路径），命中零槽位；miss 且限流满 → `429`（与 §7.8 一致）。
- **禁用**：`-cache-size 0` 关闭缓存与去重，回退 §7.8 直通行为。
- **边界**：缓存属基础设施（`internal/cache`），领域/应用层不感知。
...
| OQ2 | 多实例部署下的全局限流（进程内信号量不跨实例） | 本期单实例进程内治理；多实例走网关/共享令牌桶，后续 |
...
| v0.5 | 2026-09-03 | 新增 §7.8 并发治理与过载保护（WS-A：有界并发+排队+429/Retry-After+取消传播+优雅关闭+可配置）；§6 增过载保护决策行；§11 增 OQ2 多实例限流 |
| v0.6 | 2026-09-03 | 新增 §7.9 缓存与去重（WS-C：磁盘 LRU + single-flight 去重 + sidecar 元数据 + 启动扫描 + 命中零上游/零槽位）；§6 增缓存决策行 |
